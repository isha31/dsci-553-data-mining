...
def main():
    start = time.time()
    spark = SparkSession.builder.appName("assign2_task1").master("local[*]").getOrCreate()
    ...
    args = sys.argv
    case_no,support,inp_file,out_file = int(args[1]),int(args[2]),str(args[3]),str(args[4])

    input_rdd = sc.textFile(inp_file).map(lambda x : x.split(","))
    son(input_rdd,support,case_no, out_file)
    end = time.time()
    print("Duration: ",round(end - start,2))

def son(input_rdd,support,case_no, out_file):
    header = input_rdd.first()
    if case_no == 1:
        baskets = input_rdd.filter(lambda x: x != header).map(lambda x : (x[0],frozenset( [x[1]] ) )).reduceByKey(lambda x,y : x | y).map(lambda x: x[1])
    elif case_no == 2:
        baskets = input_rdd.filter(lambda x: x != header).map(lambda x : (x[1],frozenset([x[0]]))).reduceByKey(lambda x,y : x | y).map(lambda x: x[1])
